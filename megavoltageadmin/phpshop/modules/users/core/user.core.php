...
class PHPShopUser extends PHPShopCore {
            ...
            if (is_array($row)) {
                ...
                if (is_array($content))
                    foreach ($content as $k => $v) {
                        ...
                        $dop.=ParseTemplateReturn($GLOBALS['SysValue']['templates']['users']['user_forma_dop_content'], true);
                    }

                // Определяем переменные
                $this->set('userContent', $dop);
                $this->set('pageContent', ParseTemplateReturn($GLOBALS['SysValue']['templates']['users']['users_forma_enter'], true));
    ...
    function enter($activation = false) {
        ...
        if (!empty($activation)) {
            ...
            if ($this->option['mail_check'] == 1)
                $this->set('activationNotice', PHPShopText::notice($GLOBALS['SysValue']['lang']['users_notice']));
            else
                $this->set('activationNotice', PHPShopText::notice($GLOBALS['SysValue']['lang']['users_notice_admin_check']));
        }

        // Определяем переменные
        $this->set('pageContent', ParseTemplateReturn($GLOBALS['SysValue']['templates']['users']['users_forma'], true));
    ...
    function sendpassword() {
        ...
        // Определяем переменные
        $this->set('pageContent', ParseTemplateReturn($GLOBALS['SysValue']['templates']['users']['users_forma_lost'], true));
...
';
    ...
    function register() {
        ...
        // Определяем переменные
        $this->set('pageContent', ParseTemplateReturn($GLOBALS['SysValue']['templates']['users']['users_forma_register'], true));
    ...
    function update_user() {
        ...
        if (PHPShopSecurity::true_passw($_POST['password']) and PHPShopSecurity::true_email($_POST['mail'])) {
    ...
    function add_user() {
        ...
        // Проверка каптчи
        if ($this->option['captcha'] == 1 and $_POST['key'] != $_SESSION['text']) {
