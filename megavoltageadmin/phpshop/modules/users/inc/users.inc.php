...
class PHPShopUsersElement extends PHPShopElements {
    ...
    function autorizationForma() {
        ...
        if($this->is_autorization()) {
            ...
            $this->set('leftMenuContent',ParseTemplateReturn($GLOBALS['SysValue']['templates']['users']['users_forma_element'],true));

        }
        else {
            $this->set('leftMenuName','Авторизация');
            $this->set('leftMenuContent',ParseTemplateReturn($GLOBALS['SysValue']['templates']['users']['users_forma'],true));
        }
