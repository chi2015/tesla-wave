...
function actionStart() {
    ...
    if(isTypeAllowed($tpl_file)) {
        ...
        $code='<textarea name="code" id="code" style="width:100%;height:600px">'.$content.'</textarea>';
        $code.='    <script>
...
function actionUpdate() {
    ...
    if(isTypeAllowed($tpl_file)) {
        ...
        if(fwrite(fopen($tpl_file,"w+"), $_POST['code'])) $return='Файл ['.str_replace('../../..','',$tpl_file).'] сохранен.';
...
<html>
    <head>
        ...
        <LINK href="<?=$_classPath.'admpanel/'?>skins/<?=$PHPShopSystem->getSerilizeParam("admoption.theme")?>/texts.css" type=text/css rel=stylesheet>
