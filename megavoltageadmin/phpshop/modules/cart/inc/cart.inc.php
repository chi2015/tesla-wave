...
// Настройки
$GLOBALS['_ADMIN']['dir']="./UserFiles/Price/";
...
class PHPShopCartArray extends PHPShopArray {
    function PHPShopCartArray() {
        ...
        parent::PHPShopArray("filedir","catdir","enabled","email","valuta","enabled_market","num",'enabled_speed','enabled_search');
    }
}

function speed_check() {
    if(empty($GLOBALS['LoadItems']['modules']['cart']['enabled_speed'])) return true;
    elseif($GLOBALS['SysValue']['nav']['path'] == "price") return true;
    else return false;
}
...
if(empty($output['num'])) $output['num']=1;
...
if($PHPShopNav->getPath() != "order")
    ...
    } else $PHPShopCartElement->init('miniCart');
