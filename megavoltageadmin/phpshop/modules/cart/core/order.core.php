	...
	<td colspan="2"><strong>'.$sum.'</strong> '.$GLOBALS['LoadItems']['modules']['cart']['valuta'].'</td>
        ...
        if(count($_SESSION['CART'])>0)
            $dis.='<p><input type="hidden" value="'.$this->order_num.'" name="order_num">
                <input type="button" value="Прайс-лист" onclick="window.location.replace(\'../price/\')"> <input type="button" value="Продолжить покупку" onclick="javascript:history.back(1);"> <input type="submit" name="order" value="Оформить заказ >"></form></p>';
        else $dis.='</form>';
