...
class fase4_rdf {
    ...
    var $_cache_type = "";
    var $_remote_file = "";
    var $_cache_dir = "UserFiles/Cache/";  // String
...
class PHPShopRssParser {
  ...
  FROM ".$SysValue['base']['table_name24']." as f LEFT JOIN ".$SysValue['base']['table_name25']." as s
  ...
  ($sec_day/(f.day_num+1)) < ($date - f.last_load)
    ...
    function add_rss_jurnal($link_id, $status, $last_date, $day_num ) {
        ...
        $sql = "
  UPDATE ".$SysValue['base']['table_name24']."
  set
  last_load = '$date' WHERE id = '$link_id'
        ...
        $sql = "
  INSERT INTO ".$SysValue['base']['table_name25']."
  VALUES ('','$date','$link_id','$status')
    ...
    WHERE title LIKE '$title' LIMIT 1
            ...
            if ($n == 0) {
                ...
                // Подробное описание
                $content = $description.'<br><a href="'.$news[$i]['link'].'" target="_blank">Источник...</a>';

                // Массив данных
                $data_array=array('date_new'=>$date,'title_new'=>$title,'description_new'=>$description,'content_new'=>$content);
