...
class PHPShopLang {
     ...
     * Конструктор
     */
    function PHPShopLang($_classPath,$_localePath=false) {
        ...
        if(!empty($_SESSION['lang'])) {
            ...
            if(empty($_localePath)) $_localePath='admpanel/locale/';

            // Чтение локали файла
            $this->langFile = $_classPath.$_localePath.$_SESSION['lang'].'.ini';
...
';
            if($this->doLang) {
                ...
                else echo "Error update ".$this->langFile;
            }
        }
    }
...
$GLOBALS['PHPShopLang'] = new PHPShopLang(@$GLOBALS['_classPath'],$GLOBALS['_localePath']);
